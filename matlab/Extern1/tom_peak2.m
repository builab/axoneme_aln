function [varargout] = tom_peak2(varargin)
...
%
%  09/11/02   AL, /10/20/02 tested and bug fixed FF
%  add subpixel accuracy, done by GS
%  03/03/04 added different syntax of tom_sphere FF
%  last changes:11.10.04 FF
%
%  04/10/07 HB, boxed for cc peak picking
...
switch (nargin)
    ...
    case  2
        ...
        elseif length(varargin{2}) > 1
                ...
                if dim3 == 1
                    if length(box) ~=4
                        error('box limit must be [xmin xmax ymin ymax]');
                    end
                
                    mask(box(3):box(4),box(1):box(2)) = 1;
                    a = (1-mask).*min(min(min(a))) + a.*mask;
                    v=max(max(a));
                    [s1 s2]=find(a==v);
                    c=[s1 s2];
                    varargout{1}=c;
                    ...
                    if length(box) ~=6
                        error('box limit must be [xmin xmax ymin ymax zmin zmax]');
                    end
  ...
  % -------------------------------------------------------------------------              
        ...
        elseif strcmp(class(varargin{2}),'double')               % no interpolation
            % radius given
