...
%maximum value
fwrite(fid,Data.max,'float32');

%minimum value
fwrite(fid,Data.min,'float32');

%average value
fwrite(fid,Data.avg,'float32');

%standard deviation. A value of -1.0 indicates that sig has not been computed previously.
fwrite(fid,Data.std,'float32');
...
if (iform > 0) % work good
    ...
    data_real = real(Data.data);
    data_imag = imag(Data.data);
    data2_real = zeros(1, Data.y*Data.z*Data.x);
    data2_imag = zeros(1, Data.y*Data.z*Data.x);
    for i = 1:Data.z
        page_r = data_real(:,:,i)';
        page_i = data_imag(:,:,i)';
        istart = (i-1)*Data.y*Data.x + 1;
        iend = i*Data.y*Data.x;
        data2_real(istart:iend) = reshape(page_r, 1, Data.y*Data.x);
        data2_imag(istart:iend) = reshape(page_i, 1, Data.y*Data.x);
    end

    data = zeros(1, Data.y*Data.z*nsam);
    data(1:2:Data.y*Data.z*nsam) = data2_real;
    data(2:2:Data.y*Data.z*nsam) = data2_imag;
