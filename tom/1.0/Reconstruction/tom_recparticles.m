...
% TOM_RECPATICLES is used to reconstruct a particle with the hight resolution.
...
%
%      
%   Syntaxe: tom_recparticles 
%       Input :
%           -                         
%       Output:
%           -
%   
%   See also: TOM_PARTICLES ,TOM_RECONSTRUCTION3D, TOM_REC3D 
%
%   Max Planck Institut
%   20/05/2003       WDN
%
...
%for lauf=1:87 
 ...
 %   disp(['Read in TEMPV_BPP2048_' num2str(lauf) ' tiltangle of ' num2str(Projection.Header.Tiltangle)]);
    ...
    Number_of_parallel_particles=floor(size(handles.ListMarkerPoint,1)./handles.ParaRecNum);
    ...
    for i=1:handles.ParaRecNum:Number_of_parallel_particles
        ...
        for lauf=1:handles.ParaRecNum
            Volume(:,:,:,lauf)=single(zeros(handles.Sizex,handles.Sizey,handles.Sizez));
        end
        ...
        for lauf=str2num(handles.WeightedProjection.First):str2num(handles.WeightedProjection.Last)
            ...
            for lauf2=1:handles.ParaRecNum
                ...
                tom_backproj3d(single(v),single(Projection.Value),0,Projection.Header.Tiltangle,[p1 p2 p3]);
                Volume(:,:,:,lauf2)=v;
            end
        ...
        for lauf3=1:handles.ParaRecNum
            ...
            tom_emwrite(temp,squeeze(Volume(:,:,:,lauf3)));
        end
    ...
    % reconstruction of the rest
        for i=Number_of_parallel_particles+1:Number_of_parallel_particles+Number_of_rest_particles
            ...
            for lauf=str2num(handles.WeightedProjection.First):str2num(handles.WeightedProjection.Last)
                ...
                tom_backproj3d(single(Volume),single(Projection.Value),0,Projection.Header.Tiltangle,[p1 p2 p3]);
            end
            temp=[handles.Path handles.File '_' num2str(i) handles.Ext];
            tom_emwrite(temp,Volume);
        end
        %end
