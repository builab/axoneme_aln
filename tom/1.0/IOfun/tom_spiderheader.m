function Data = tom_spiderheader(in, iform)
%TOM_SPIDERHEADER adds a  Spider header structure to a matrix
%
%   Data = tom_spiderheader(in, iform)
%
...
if isstruct(in)~=1
    ...
    if nargin < 2 && size(in,3) == 1
        iform = 1;
    elseif (nargin < 2)
        iform = 3;
    end

	 Data.Header.Spider.iform = iform;
    ...
    %number of pixels per line. (size of volume in y direction)
	 if iform > 0
		...
		Data.Header.Spider.nsam = 2*size(in,2);
	end
