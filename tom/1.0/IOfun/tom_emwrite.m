...
if isstruct(Data)~=1 & isequal(form,'standard')
    if isequal(computer,'PCWIN') | isequal(computer,'GLNX86') | isequal(computer, 'GLNXA64')
...
if size(Data.Header.Comment,1)~=80
    f=char(ones((80-size(Data.Header.Comment,1)),1).*32);
    f=strvcat(Data.Header.Comment,f);
    Data.Header.Comment=f;
end
fwrite(fid,Data.Header.Comment,'char');

if size(Data.Header.EM.Parameter,1)~=40
    Data.Header.Parameter=ones(40,1);
end
fwrite(fid,Data.Header.EM.Parameter,'int32');

if size(Data.Header.Fillup,1)~=256
