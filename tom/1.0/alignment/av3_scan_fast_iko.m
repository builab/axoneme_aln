function av3_scan_fast_iko(refilename, motlfilename, particlefilename, startindx, iterations,angincr, ...
    angitr1,angitr2,angiter,mask,hipass,lowpass,nfold,threshold,wedgelist,iclass, ibin, dispflag, limit)
% av3_scan_fast_iko aligns 3D subtomograms to reference for icosahedral
%   symmetry
%
%   av3_scan_fast_iko (refilename, motlfilename, particlefilename, startindx, iterations,angincr,...
%       angitr1,angitr2,angiter,mask,hipass,lowpass,nfold,threshold,wedgelist,iclass, ibin, dispflag, limit)
...
%   angincr             angular increment, for psi angle (set to zero to omit)
%   angitr1            angular increment, for phi, No. of scans over range
%                                          of 69.09 deg
%   angitr2            angular increment, for the, No. of scans over range
%                                          of 31.72 deg
%   angiter             iterations for psi 
...
% tested KG & MB
% 
% adapted for ikosaedrical symmetry, 13/02/06
...
for ind = startindx:startindx+iterations-1, % particle loop
    name = [refilename '_' num2str(ind) '.em'];
    ref = tom_emread(name);
    ...
    itomo_old = 0;
    for indpart = 1:size(motl,2), % class loop
        if ((motl(20,indpart) == 1) | (motl(20,indpart) == 2) | (motl(20,indpart) == iclass))
            ...
            if nargin > 18, % throw away the gold, modified 14/11/05, MB
            ...
            for phi = phi_old:69.09/angitr1:phi_old+69.09, %phi loop 69.09deg?
                for ithe =  0:31.72/angitr2:31.72, %the loop 31.72deg?
                    if ithe == 0
                        npsi=1;
                        dpsi=360;
                    else
                        %sampling for psi and the on unit sphere in rings
                        dpsi=(31.72/angitr2)/sin(ithe*(31.72/angitr2)/180*pi);
                        npsi = ceil(360/dpsi);
                    end;
                    ipsi=0; %for ipsi = 0:(npsi-1), %psi loop 
                        r = [ 0 0 1];
                        r = tom_pointrotate(r,0,ipsi*dpsi,ithe*31.72/angitr2);
