function av3_scan_real(refilename, motlfilename, particlefilename, startindx, iterations, nshifts, ...
    angiter, angincr, mask, hipass,lowpass, nfold, threshold, wedgelist)
%
%   av3_scan_real(refilename, motlfilename, particlefilename, startindx, iterations, nshifts, ...
%       angiter, angincr, mask, hipass,lowpass, nfold, threshold, wedgelist)
%
%   AV3_SCAN_REAL exclusively performs an alignment in angular space.
%   Therefore, the correlation is carried out in real space. However, I
%   still prefer simultaneous rotational and translational alignment
%   (AV3_TRANS-ROT-ALIG).
%
%  SEE ALSO
                ...
                for ishfty=-nshifts:nshifts
                    for ishftz=-nshifts:nshifts
                        ...
                        if (radius >= nshifts)
                            ...
                            shift_part = tom_move(particle,-shift);
                            % angle loop
                            for phi = phi_old-angiter*angincr:angincr:phi_old+angiter*angincr
                                for ithe =  0:angiter
                                    ...
                                    for ipsi = 0:(npsi-1)
                                        ...
                                        rot_part = double(tom_rotate(shift_part,[-psi,-phi,-the]));
                                        rot_part = mask.*tom_symref(rot_part,nfold);
                                        %tom_dspcub(shift_part);
                                        mpart = sum(sum(sum(rot_part(imask))))/npixels;
                                        rot_part(imask) = rot_part(imask) - mpart;
                                        pvar = sqrt(sum(sum(sum(rot_part(imask).*rot_part(imask)))));
                                        rot_part(imask) = rot_part(imask)/pvar;
                                        ccctmp = sum(sum(sum(rot_part(imask).*tref(imask))));
                                        if ccctmp > ccc
                                            ccc = ccctmp;
                                            phi_opt=phi;
                                            psi_opt=psi;
