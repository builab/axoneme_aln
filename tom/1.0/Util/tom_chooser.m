...
% TOM_CHOOSER "classifys" particles in tomogram based on a motivelist
...
%   DSPCUBMODE : mode for tom_dspcub (0,1,2)
...
tom_dspcub(tom_filter(particle,handles.ifilt,'circ'),handles.dspcubmode); 
axes(handles.proj);colormap gray;
particle = sum(particle,3);

imagesc(particle');

% UIWAIT makes tom_chooser wait for user response (see UIRESUME)
uiwait(gcf);
