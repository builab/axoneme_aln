...
bandpassmask = ones(size(mask,1),size(mask,2),size(mask,3));
bandpassmask = tom_spheremask(bandpassmask,lowpass,3) - tom_spheremask(bandpassmask,hipass,2);;
if nargin < 8 
    iclass = 0;
end;
icount = 0;
...
for indpart1 = 1:size(motl,2) 
    if ( (motl(20,indpart1) == iclass) | (motl(20,indpart1) == 1) | (motl(20,indpart1) == 2) )
        ...
        frpart1 = bandpassmask.*frpart1;
        tmpwei1 = tom_maskrotate(wedge,[-psi,-phi,-the]);
