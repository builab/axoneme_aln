function varargout = tom_particles(varargin)
% TOM_PARTICLES(VOLUME) a tool for interactive 3D particles picking 
%
%   tom_particles(volume) This function can be used to pick particles from
...
%  
%   Syntaxe: tom_particles or tom_particles(1,'fixed')
%            tom_particles(1,Filename) or tom_particles(1,Filename,'fixed') 
%       Input :
...
%       tom_particles;
%       tom_particles or tom_particles(1,'fixed')
%       tom_particles(1,'fixed noinfo');
%   
%   You want to give as parameter the file's name
%       tom_particles(1,'C:\MATLAB6p5\work\mem_2bin1_vol.em');
%       tom_particles(1,'C:\MATLAB6p5\work\mem_2bin1_vol.em','fixed');
%
%   See also TOM_VOLXY, TOM_INTERVOL
%
%   Created: 13/11/03 William Del Net
%   Last modification: 04/02/04 William Del Net 
%   
%    Copyright (c) 2004
...
% Edit the above text to modify the response to help tom_particles

% Last Modified by GUIDE v2.5 04-Feb-2004 11:13:09

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @tom_particles_OpeningFcn, ...
                   'gui_OutputFcn',  @tom_particles_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before tom_particles is made visible.
function tom_particles_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to tom_particles (see VARARGIN)

% Choose default command line output for tom_particles
...
guidata(hObject,handles);
clear tmpp;
% UIWAIT makes tom_particles wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = tom_particles_OutputFcn(hObject, eventdata, handles)
