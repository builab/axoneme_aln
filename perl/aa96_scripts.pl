...
for ($scriptType) {
	...
	/^15$/ && do {	
		...
		last;};
	/^16$/ && do {
		$params{"EllipseDistortionThreshold"} = getInput("Ellipse Distortion Threshold (1.10 very round; 1.15 ok)", $params{"EllipseDistortionThreshold"});
		$params{"RsCalculationLowerBound"} = getInput("Lower bound for calculation", $params{"RsCalculationLowerBound"});
		$params{"RsCalculationUpperBound"} = getInput("Upper bound for calculation", $params{"RsCalculationUpperBound"});
		&writeParams(\%params, $paramFile);
                &writeTemplateToMScript("${scriptDir}/${projName}_plotRsOrganization.m", "${templateDir}/plotRsOrganization.template", $paramFile);
                last;};
	/^17$/ && do {&writeTemplateToMScript("${scriptDir}/${projName}_adjustOrigin.m", "${templateDir}/adjustOrigin.template" , $paramFile); last;};
	/^done$/ && do {
		$isNotDone = 0;
		last;
	};
...
sub writeAvgScript {
	...
	if ($doAvgDoublet) {
		for (my $i = 1; $i <= 9; $i++) {
			...
			$outputSubList =~ s/\.txt$/_d$i\.txt/i;
			$outputSubList =~ s/_ida_v1_d$i\.txt/_d$i\.txt/i;
			my $output = $params{"AverageFile"};
			$output =~ s/\.spi$/_d$i\.spi/i;
			$output =~ s/_avg_d$i\.spi$/_d${i}_avg\.spi/i;
			$output =~ s/_ida_v1_d$i/_d$i/i;
...
sub writeIdaScript {
	...
	print FILEHDL "nohup batch_job_submit.pl $numberOfProcessors ./$scriptDir/${projName}_shiftIdaAln.bat > ${projName}_shiftIdaAln.log\n";
