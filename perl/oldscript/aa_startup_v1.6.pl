...
$params{"Reference"} = getInputPersistence("Reference File", "${projName}_ref_masked.spi");
#$params{"LowResReference"} = getInputPersistence("Low Resolution Reference File", "${projName}_ref_low.spi");
$params{"Mask"} = getInputPersistence("Mask File", "${projName}_3dmask.spi");
