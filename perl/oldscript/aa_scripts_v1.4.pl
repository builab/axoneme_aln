...
for ($scriptType) {
	...
	/^5$/ && do {&writeTemplateToMScript("${scriptDir}/${projName}_roughAln.m", "${templateDir}/roughAln.template", $paramFile); last;};
	...
	/^12$/ && do {&writeAvgScript("${scriptDir}/${projName}_avg.bat" , $listFile, $paramFile); last;}; 
	/^13$/ && do {&writeTemplateToMScript("${scriptDir}/${projName}_calcMwWeight.m", "${templateDir}/calcMwWeight.template" , $paramFile); last;};
	/^14$/ && do {&writeTemplateToMScript("${scriptDir}/${projName}_weightedAvg.m", "${templateDir}/weightedAvg.template" , $paramFile); last;};
	/^15$/ && do {&writeCleanUpScript("${scriptDir}/${projName}_cleanUp.bat"); last;};
	/^16$/ && do {
...
sub writeOdaScript {
	...
	print FILEHDL "nohup batch_job_submit.pl $numberOfProcessors ./$scriptDir/${projName}_intraAln.bat > intraAln.log\n";
	...
	print FILEHDL "./$scriptDir/${projName}_cbnDoc.bat > cbnDoc.log\n";
	print FILEHDL "nohup batch_job_submit.pl $numberOfProcessors ./$scriptDir/${projName}_interAln.bat > interAln.log\n";
	print FILEHDL "./$scriptDir/${projName}_cbnAllDoc.bat > cbnAllDoc.log\n";
	print FILEHDL "nohup batch_job_submit.pl $numberOfProcessors ./$scriptDir/${projName}_avg.bat > avg.log\n";
