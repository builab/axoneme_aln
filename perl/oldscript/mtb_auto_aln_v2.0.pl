...
# 1.91: fix problem of filename < 80 characters in SPIDER
# 2.0: now allow binning for faster processing
use Cwd;
use Data::Dumper;

$scriptName = "MTB_AUTO_ALN_BIN.PL";
$version = '2.0';
$vdate = '2014/03/05';
...
# Associated script
$spider_aln_script = 'spider_aln_bin.pl';
...
$iter = 1;
$searchType = 0;
$processingDir = '.'; # TO CHANGE OR PUT INTO COMMAND LINE OPTION
$startingDir = getcwd;
$bin = 1;
...
if ($doAlign) {
    $doc_prefix_tmp = $doc_output_prefix;
    $doc_prefix_tmp =~ s/^doc\///i;
    $args{"-doc_prefix"} = $doc_prefix_tmp;
    $args{"-box"} = $box;
...
if ($doc_init ne '') {
    $doc_init_tmp = $doc_init;
    $doc_init_tmp =~ s/^doc\///i;
    $args{"-doc_init"} = $doc_init_tmp;
}
...
if ($ref ne '') {
	system("cp $startingDir/$ref .");
}

if ($doc_init ne '') {
	print "ln -s ${startingDir}/$doc_init .\n";
	system("ln -s ${startingDir}/$doc_init .");
}
...
if ($doCmd) {
	...
	print("mv ${doc_prefix_tmp}${output}.spi ${startingDir}/doc\n");
	system("mv ${doc_prefix_tmp}${output}.spi ${startingDir}/doc");
	chdir $startingDir;
	system($rmCmd);
}
...
sub print_usage {
   ...
   print "Usage: mtb_auto_aln_init.pl -flag option output\n";
	...
	print " -bin 2\t using bin for faster calculation\n";
}
...
sub pickDataBsoft {
	...
	# PICK	
	my $pick_cmd = "bpick -split -extract " . $extract_radius . ',' . $extract_radius . ',' . $extract_radius . ' -background -normalize -extension spi -base ' . $tmp_data . '_ ' . $corr_star_file;
	print "$pick_cmd\n";	
