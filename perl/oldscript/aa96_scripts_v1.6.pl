...
sub writeIntraScript {
	...
	for ($i = 1; $i <= 4; $i++) {
		...
		print FILEHDL "#END_PARALLEL\n";
		print FILEHDL "mv doc_intra_*_ida_v$i* doc\n";
		print FILEHDL "rm doc_init_*_ida_v$i* doc\n";
		close FILEHDL;
		# Set permission to run
		&finishWriting($scriptFile);	
	}
...
sub writeAvgScript {
	...
	if ($doAvgDoublet) {
		...
		my $output = $params{"AverageFile"};
		print FILEHDL "spider_avg.pl -output $output -list $listFile\n";
	}
	
	print FILEHDL "rm doc_total_*\n";
	close FILEHDL;
	# Set permission to run
	&finishWriting($scriptFile);
	return 1;
