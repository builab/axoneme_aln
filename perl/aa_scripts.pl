...
#	add printMenu
# v1.8  add redrawModel, regenerateStarFile	
...
%addMenu = (	17 => "Supervised Classification",
		18 => "Reorder Axoneme",
		19 => "Redraw Model",
		20 => "Regenerate Star Files");
	


if ($#ARGV < 2 && $#ARGV != -1) {
	print "Usage: aa_script.pl listFile paramFile scriptType\n";
...
for ($scriptType) {
	...
	/^17$/ && do {
		...
		$params{"PlotClassificationResultsAln"} = getInput("Draw results with doublet alignment (Yes=1, No=0)", 0);
		$iter = $params{"IterationNo"};
		$noRef = $params{"NumberOfReferences"};
		&writeParams(\%params, $paramFile);
		&writeTemplateToMScript("${scriptDir}/${projName}_multirefClassification_ref${noRef}_iter${iter}.m", "${templateDir}/multirefClassification.template", $paramFile);
		&writeTemplateToMScript("${scriptDir}/${projName}_plotClassificationResult_ref${noRef}_iter${iter}.m", "${templateDir}/plotClassificationResult.template", $paramFile);
		last;
	};		
	...
	/^20$/ && do {
		print "\n*****WARNINGS: running this script will overwrite your old star files and doc_init files. Please back up before running!!!*****\n";
                &writeTemplateToMScript("${scriptDir}/${projName}_regenerateStarFile.m", "${templateDir}/regenerateStarFile.template", $paramFile);		
                        last;};
...
sub writeIntraScript {
	...
	my $generalArg = ' -doc_output_prefix ' . $docDir . '/doc_intra_' . ' -bin 2 ';
...
sub writeUseFittedStarScript {
	...
	while (<LSTHDL>) {
		...
		if (/\#/) { next; }
		my @line = split(' ', $_);
		my $starFile = $starDir . '/' . $line[1] . '.star';
		my $fittedFile = $starFile;
