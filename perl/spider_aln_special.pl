...
# Identical to 1.6
# In averaging mode, it will output the align particles

$scriptName = "SPIDER_ALN_SPECIAL.PL";
$version = '1.8s';
$vdate = '2010/11/09';
...
# default argument
$SPIDER_BIN = 'spider_linux_mp_opt64';
$ref = '';
...
if ($searchType == 1) {
	...
	if ($doAlign) {
		&print_spider_aln($soc_script, $name, $start, $end, $output, $ref, \@bandpass, $docOutputPrefix, \@box, $searchRadius, $exclude, $docInit);
	} else {
...
sub print_spider_aln {
	...
	my ($soc_script, $name, $start, $end, $output, $ref, $bandpass, $docOutputPrefix, $box, $searchRadius, $exclude, $docInit) = @_;
...
sub print_spider_avg {
	...
	if ($docInit ne '') {
		$name =~ s/(\*+)/\{$1\[particle_id\]\}/g;
		$alnpart =~ s/(\*+)/aln_\{$1\[particle_id\]\}/g;
	}
...
FR G
[alnpart]${alnpart} ; Output aligned particle
